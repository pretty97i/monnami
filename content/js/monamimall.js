...
$(document).ready(function(){
	...
	*/
	...
	function headerScroll(){
		...
		/* 윙배너 숨김처리
		if ( path2 == "main" ){
			if ( headerst < banwingtop + $(".banner-wing.active").height() + 40 - $(window).height() ){
				$(".banner-wing").css({"position":"absolute", "top":+(banwingtop)+"px","bottom":"auto"});
			}else{
				$(".banner-wing").css({"position":"fixed", "top":"auto","bottom":"40px"});
			}
		}
		*/
	...
	if ( path2 == "sub" ){
		...
		if ( path3 == "goods detail" ){
			...
			if($('.swiper-container.howtouse-top').length){
				setTimeout(function(){
		...
		if ( path3 == "mypage active" ){
			...
			/* 상품리뷰 주석
			var fileTarget = $('.file-areacomm .upload-hidden');
