...
<link rel="stylesheet" type="text/css" href="<%=cp %>/monnami/css/common.css" />
<link rel="stylesheet" type="text/css" href="<%=cp %>/monnami/css/main.css">
<link rel="stylesheet" type="text/css" href="<%=cp %>/monnami/css/products.css" />
<link rel="stylesheet" type="text/css" href="<%=cp %>/monnami/css/contents.css" />
...
<script src="<%=cp %>/monnami/js/swiper.js"></script>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$(".datecalform").datepicker("setDate", "2019.02.12");
		$(".datecalto").datepicker("setDate", "2019.03.12");
	});
	...
	function setMonth(val){
		...
		$(".datecalform").datepicker("setDate", periodStr);
		$(".datecalto").datepicker("setDate", getDate("1","."));
	}

	
	function goSearch(){
		if($("#from").val() > $("#to").val()){
			alert("조회기간 종료일이 시작일보다 빠릅니다!");
			$("#to").focus();
			return false;
		}
		
		var frm = document.orderForm;
		frm.action="/w/mypage/order/myClaimList.do";
		frm.submit();
	}
	...
	function goDetail(claimIdx){
		var frm = document.orderForm;
		$("#claimIdx").val(claimIdx);
		frm.action = "/w/mypage/order/claimDetail.do";
		frm.submit();
	}

	
	function popInvoice(orderNo, claimIdx){
		$.ajax({			
			url: getContextPath()+"/ajax/order/invoiceLayer.do",
			data: {
				"orderNo"	: orderNo,
				"claimIdx"	: claimIdx
			},
			...
			success: function(data){
				$("#popOrderInquiry .popinner .contents .orderinquiry .scroll-content").html(data);
				fn.popupOpen('#popOrderInquiry');
			 }
		});		
	}
...
//		 		alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error); 
	...
	<!-- contents// -->
	<main id="contents" class="mypage payment" data-index="1">
		<div class="inner">
			...
			<a href="/w/mypage/order/main.do">마이페이지 메인</a>
					...
					<li class="lnb-depth1 active" data-page="payment">
						...
						<ul class="lnb-depth2">
							...
							<li class="active"><a href="/w/mypage/order/myClaimList.do">취소/교환/반품조회</a></li>
				...
				<div class="cont-area inquire">
					<h3 class="tit">취소/교환/반품조회</h3>

					<!-- 조회기간 검색// -->
					<form name="orderForm" id="orderForm" method="post">
						<input type="hidden" name="pageNo" id="pageNo" value="1"/>
						<input type="hidden" name="claimIdx" id="claimIdx" value=""/>
						...
						<fieldset>
							...
							<div class="date-form">
								...
								<div class="select-area">
									<p>
										<b>2019.02.12 ~ 2019.03.12</b> 취소/교환/반품 내역 입니다.
									</p>
					...
					<!-- 취소/교환/반품안내// -->
					<div class="info-step">
						<h4 class="tit">취소/교환/반품안내</h4>
						<div class="area">
							<strong class="tit">주문취소</strong>
							<ul class="txt">
								<li>
									・ 주문취소는 결제완료 단계까지 가능합니다. 다만, 주문취소가 불가능한 상품이 존재하오니, 상품 주문 시 꼭 유의해 주시기 바랍니다.
								</li>
							</ul>
						</div>
						<div class="area">
							<strong class="tit">교환단계 안내</strong>
							<ol class="step">
								<li>
									<dl>
										<dt>Step 01. <b>교환신청</b></dt>
										<dd>
											요청하신 상품에 대해<br>
											교환신청이 완료되었습니다.
										</dd>
								...
								<li>
									...
									<ul class="txt">
										<li>
											・  고객발송 : 고객님께서 직접 상품을<br>
										    반송지로 보내주셔야 합니다.<br>
									    	반송지 주소는 고객센터에서 안내해<br>
										    드릴 예정입니다.
										</li>
									</ul>
								</li>
						...
						<div class="area">
							<strong class="tit">반품단계 안내</strong>
							<ol class="step">
								<li>
									<dl>
										<dt>Step 01. <b>반품신청</b></dt>
										<dd>
											요청하신 상품에 반품신청이<br>
											완료되었습니다.
										</dd>
								...
								<li>
									<dl>
										<dt>Step 02. <b>반품 진행</b></dt>
										<dd>
											반품 상품을 회수 중에 있습니다.<br>
											반품 상세 페이지에서 보다<br>
											자세한 반품 방법을 확인해<br>
											주시기 바랍니다
										</dd>
								...
								<li>
									<dl>
										<dt>Step 03. <b>반품완료</b></dt>
										<dd>
											반품상품에 대한 환불이<br>
											완료된 상태 입니다.<br>
											결제수단에 따라 환불 소요기간은<br>
											다소 차이가 날 수 있습니다
										</dd>
									</dl>
								</li>
							...
							<div class="box">
								<ul class="txt">
									<li>
										・ 택배기사방문 : 택배기사가 직접 회수지로 상품을 수거하기 위해 방문할 예정입니다. 2~3일 이후에도 택배기사로 부터 연락이 없으시면 고객센터로<br> 문의주시기 바랍니다.
									</li>
									<li>
										・ 고객직접발송 : 고객님께서 직접 상품을 반송지로 보내주셔야 합니다. 반송지 주소는 교환상세/반품상세 화면에서 확인하실 수 있습니다
									</li>
								</ul>
							</div>
						</div>
						<div class="area">
							<strong class="tit">교환/반품기간</strong>
							<ul class="txt">
								<li>
									・ 상품 및 서비스를 공급 받으신 날로부터 7일 이내에 교환 및 반품이 가능합니다.
								</li>
								<li>
									・ 재화 등의 내용이 표시, 광고 내용과 다르거나 계약내용과 다르게 이행된 경우에는 재화 등을 공급받은 날로부터 1개월 이내,<br>
									그 사실을 안 날 또는 알 수 있었던 날부터 0일 이내에 교환 및 반품이 가능합니다.
								</li>
							</ul>
							<strong class="tit mgt33">교환/반품불가안내</strong>
							<ul class="txt">
								<li>
								...
								<li>
									・ 시간 경과에 따라 재판매가 어려운 상품 등의 가치가 현저히 감소한 경우 (신선식품의 경우 선도저하 및 상품불량일 경우 수령일로부터 1~2일 내 교환/반품 신청)
								</li>
								<li>
									・ 복제가 가능한 재화 등의 포장을 훼손한 경우(포장 인증된 전자제품, DVD,CD 도서 등 복제 가능한 상품)
								</li>
								<li>
									・ 배송된 상품이 하자 없음을 확인한 후 설치가 완료된 상품의 경우(가전,가구,헬스기구 등)
								</li>
								<li>
									・ 고객님의 요청에 따라 개별적으로 주문제작되는 상품의 경우(수제화, 귀금속 등)
								</li>
								<li>
									・ 구매한 상품의 구성품이 누락된 경우(화장품 세트, 의류 부착 악세서리, 가전제품 부속품,사은품 등)
								</li>
								<li>
