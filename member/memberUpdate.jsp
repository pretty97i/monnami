...
<script>
	...
	function memberModify(){
		...
		if($("#memberEmailChk").val() != $("#memberEmail").val()){
		...
		if($("input:checkbox[id='defaultAddrChk']").is(":checked")){
			if($.trim($("#zipCode").val()) == "" && $.trim($("#ADDR").val()) == ""){
				alert("주소가 없으므로 기본 배송지 설정을 할 수가 없습니다.");
				return false;
			}else{
				$("#defaultYn").val("Y");
			}
		...
		if($("input:checkbox[id='smsChk']").is(":checked")){
	 	...
	 	if(doneSubmit == false){					
	 		doneSubmit = true;
			var frm = document.memberInfoForm;
			frm.action = "/w/mypage/member/memberModify.do";
			frm.submit();
	 	}
	}
	...
	// CALLBACK 주소정보 값 
	var callback = function(zipCode, addr){
		$("#zipCode").val(zipCode);
		$("#addr").val(addr);
		$("#addrDetail").focus();
   	}
	...
	// 휴대폰 변경
	function hpChange(){
		authTypeCheck('hpChange');	
	}
	
	// 휴대폰 변경 값 콜백
	function hpChangeCallback(){
		var params = $("form[name=formData]").serialize();
		
		$.ajax({
		 	url : "/w/ajax/mypage/member/authorizationSaveAjax.do", 
			data :	params,	
			 type : "POST",	
			 async : false,
			 cache : false,	 		
			 ...
			 success : function(info){
				 if(info.flag == 2){
					var frm = document.authMemberForm;
					frm.action = "/w/member/authMemberAlert.do";
					frm.submit();
				 }else{
					 $("#birthDate").val(info.vo.birthDate);
					 $("#ci").val(info.vo.ci);
					 $("#di").val(info.vo.di);
					 $("#memberHp").val(info.vo.memberHp);
				 }
			 }
		});
	}
...
//		 		alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error); 
	...
	<form name="authMemberForm" method="post" onsubmit="return false;">
		<input type="hidden" name="authAlertFlag" id="authAlertFlag" value="N"/>
	</form>
	<main id="contents" class="mypage members">
		<div class="inner">
...
<div class="lnb-area">
	...
	<!-- 회원 -->
					...
					<li class="lnb-depth1 " data-page="payment">
						...
						<ul clsass="lnb-depth2">
							<li ><a href="/study/monnami/member/memberUpdate.jsp">주문목록/배송</a></li>
							<li ><a href="/study/monnami/member/join.jsp">취소/교환/반품조회</a></li>
				...
				<li class="lnb-depth1 " data-page="gift">
					...
					<li class="lnb-depth1 active" data-page="members">
						<a href="javascript:">회원정보관리</a>
						<ul class="lnb-depth2">
							<li class="active"><a href="/w/mypage/member/memberInfo.do">개인정보관리</a></li>
				...
				<div class="cont-area myinfo-update">
					<h3 class="tit">개인정보관리</h3>
					<form name="memberInfoForm" method="post" onsubmit="return false;">
						<input type="hidden" name="memberEmailChkFlag" id="memberEmailChkFlag" value="N"/>
						<input type="hidden" name="memberEmailChk" id="memberEmailChk" value="jang9076@naver.com">
						...
						<input type="hidden" name="joinType" id="joinType" value="I">
						...
						<fieldset>
							...
							<table class="type2">
								...
								<tbody>
									<tr>
										<th>이름 *</th>
										<td>
											<input type="text" name="memberNm" id="memberNm" value="memberName" title="이름" readonly>
										</td>
									</tr>
									<tr>
										<th>아이디 * </th>
										<td>
											<input type="text" name="memberId" id="memberId" value="memberId" title="아이디" readonly>
										</td>
									</tr>
										...
										<tr>
											<th>비밀번호확인 * </th>
											<td>
												<input type="password" id="memberPwdChk" placeholder="비밀번호 재입력" title="비밀번호 재확인 입력" maxlength="16">
											</td>
										</tr>
									...
									<tr>
										<th>이메일 *</th>
										<td>
											<input type="text" class="w344" name="memberEmail" id="memberEmail" value="memberEmail" title="이메일 입력" maxlength="50"><button type="button" class="btn-white w138" onclick="emailDuplicateChk();">중복확인</button>
										</td>
									</tr>
									<tr>
										<th>휴대폰번호 * </th>
										<td>
											<input type="text" class="w344" name="memberHp" id="memberHp" value="memberPhone" title="휴대폰 번호 입력" maxlength="11" readonly><button type="button" class="btn-white w138" onclick="hpChange();">휴대폰변경</button>
										</td>
									</tr>
								</tbody>
								<tbody>
									<tr>
										...
										<td>
											<div class="float-area">
												<button type="button" class="btn-white w138" onclick="addrPopup();">주소찾기</button>
												<input type="text" class="w131" name="zipCode" id="zipCode" value="" title="우편번호 입력" readonly>
												<input type="text" class="w513" name="addr" id="addr" value="" title="주소 입력" readonly>
											</div>
											<div class="mgt10">
												<input type="text" class="w100" name="addrDetail" id="addrDetail" value="" title="주소 입력">
											</div>
											<p class="txt-delivery mgt10">
												<label><input type="checkbox" class="small" id="defaultAddrChk"><span>기본배송지 설정</span></label>
												<small class="bul-ty">주문상품의 배송지 주소는 배송지 관리에서 수정해주세요</small>
												<a href="/w/mypage/member/shippingList.do" class="btn-gray small">배송지관리</a>
											</p>
										</td>
									</tr>
									<tr>
										<th>생년월일</th>
										<td class="pdb19">
											<input type="text" name="birthDate" id="birthDate" value="20040306" title="생년월일 입력" readonly maxlength="8">
										</td>
									</tr>
									<tr>
										<th>수신동의</th>
										<td class="pdt11">
											<label><input type="checkbox" class="small" id="smsChk" ><span>SMS를 통한 상품 및 이벤트 정보 수신에 동의 합니다. [선택]</span></label><br />
											<label><input type="checkbox" class="small" id="mailChk" ><span>이메일을 통한 상품 및 이벤트 정보 수신에 동의 합니다. [선택]</span></label>
											<small class="bul-ty type2">SMS 및 이메일 수신동의와 무관하게 주문결제/회원 관련 안내 메일/문자는 발송되며<br />수신미동의 시 쇼핑몰 이용에 제약은 없습니다.</small>
										</td>
									</tr>
								</tbody>
							</table>
							...
							<div class="btn-area">
								...
								<button type="button" class="btn-black" onclick="memberModify();">확인</button>
							</div>
						</fieldset>
					</form>
				</div>	
...
<form name="form_auth" method="post">
	...
	<input type="hidden" name="ordr_idxx" value="TEST201903112310390000225"/>
...
<script type="text/javascript">
	...
	function getSessionId(){
		if(localStorage.getItem("monamiSessionId") == null){
			localStorage.setItem("monamiSessionId","6A8DF187E12CF4900D144CF801F8A8A0");
		}
